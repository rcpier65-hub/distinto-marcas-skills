...
export function CommandPalette({ open, onClose, marcas = MARCAS_NAV, permisos, emailActivo }: Props) {
  ...
  const actions: Action[] = useMemo(() => [
    ...
    { id: 'nav-recordings', title: 'Calendario', category: 'navegacion', icon: <IconVideo />, shortcut: ['G', 'R'], keywords: 'video shooting reunion calendario grabaciones fechas', href: '/grabaciones/calendario', requiereModulo: 'publicaciones' },
    { id: 'nav-notas-reuniones', title: 'Notas y reuniones', subtitle: 'Transcribe y pregunta sobre reuniones', category: 'navegacion', icon: <IconNote />, keywords: 'notas reuniones transcript granola micrófono chat', href: '/notas-reuniones', requiereModulo: undefined },
    ...
    { id: 'create-nota', title: 'Nueva nota', category: 'crear', icon: <IconNote />, keywords: 'apunte memo', href: '/notas-reuniones/nueva', requiereModulo: undefined },
    ...marcas.map<Action>((m) => ({
      id: `marca-${m.slug}`,
  ...
  const esPedro = (emailActivo ?? permisos?.email ?? '').trim().toLowerCase() === 'pedro@agenciadistinto.com'

  const visibles = useMemo(() => {
    let list: Action[]
    if (!permisos || permisos.rolBase === 'director') list = actions
    else {
  ...
  const filtered = useMemo(() => {
    ...
    return visibles.filter((a) =>
      a.title.toLowerCase().includes(q) || a.subtitle?.toLowerCase().includes(q) || a.keywords?.toLowerCase().includes(q) || a.category.toLowerCase().includes(q)
    )
  }, [query, visibles])
  ...
  const categoryLabels: Record<string, string> = { navegacion: 'Navegación', crear: 'Crear', accion: 'Acciones', marca: 'Marcas' }
  let globalIdx = 0

  return (
    <>
      <div onClick={onClose} className="mk-anim-fade" style={{ position: 'fixed', inset: 0, background: 'rgba(0, 0, 0, 0.50)', backdropFilter: 'blur(6px)', zIndex: 9000 }} />
      <div className="mk-anim-scale-in" style={{ position: 'fixed', top: '15vh', left: '50%', transform: 'translateX(-50%)', width: 'min(640px, 92vw)', maxHeight: '70vh', background: 'var(--mk-bg-overlay)', border: '1px solid var(--mk-border-default)', borderRadius: 'var(--mk-radius-xl)', boxShadow: 'var(--mk-shadow-xl)', zIndex: 9001, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '14px 18px', borderBottom: '1px solid var(--mk-border-subtle)' }}>
          ...
          <input autoFocus value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Buscar o ejecutar comando…" style={{ flex: 1, background: 'transparent', border: 'none', outline: 'none', color: 'var(--mk-text-primary)', fontFamily: 'inherit', fontSize: 'var(--mk-text-base)', letterSpacing: 'var(--mk-tracking-snug)' }} />
          ...
          {(['navegacion', 'crear', 'accion', 'marca'] as const).map((cat) => {
            ...
            return (
              <div key={cat} style={{ marginBottom: 4 }}>
                <div style={{ padding: '6px 14px 4px', fontSize: 10, textTransform: 'uppercase', letterSpacing: 'var(--mk-tracking-caps)', color: 'var(--mk-text-tertiary)', fontWeight: 500 }}>{categoryLabels[cat]}</div>
                {items.map((a) => {
                  const idx = globalIdx++
                  const selected = idx === selectedIdx
                  return (
                    <button key={a.id} onMouseEnter={() => setSelectedIdx(idx)} onClick={() => execute(a)} style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%', padding: '7px 12px', background: selected ? 'var(--mk-bg-selected)' : 'transparent', border: 'none', borderRadius: 'var(--mk-radius-md)', color: 'inherit', fontFamily: 'inherit', textAlign: 'left', cursor: 'pointer', position: 'relative' }}>
                      {selected && <span style={{ position: 'absolute', left: 4, top: 8, bottom: 8, width: 2, background: 'var(--mk-accent)', borderRadius: 'var(--mk-radius-full)' }} />}
                      <span style={{ width: 16, display: 'flex', justifyContent: 'center', color: selected ? 'var(--mk-accent)' : 'var(--mk-text-tertiary)' }}>{a.icon}</span>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: 'var(--mk-text-sm)', color: 'var(--mk-text-primary)', fontWeight: 500 }}>{a.title}</div>
                        {a.subtitle && <div style={{ fontSize: 'var(--mk-text-xs)', color: 'var(--mk-text-tertiary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{a.subtitle}</div>}
                      </div>
                      {a.shortcut && <span style={{ display: 'flex', gap: 2 }}>{a.shortcut.map((k, i) => (<span key={i} className="mk-kbd">{k}</span>))}</span>}
                    </button>
                  )
                })}
              </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '8px 14px', borderTop: '1px solid var(--mk-border-subtle)', fontSize: 'var(--mk-text-xs)', color: 'var(--mk-text-tertiary)', background: 'rgba(0,0,0,0.20)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}><span className="mk-kbd">↑</span><span className="mk-kbd">↓</span><span>navegar</span></div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}><span className="mk-kbd">↵</span><span>seleccionar</span></div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}><span className="mk-kbd">esc</span><span>cerrar</span></div>
          <div style={{ marginLeft: 'auto' }}>{filtered.length} resultado{filtered.length !== 1 ? 's' : ''}</div>
        </div>
      </div>
    </>
  )
