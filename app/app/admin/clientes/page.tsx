...
export default async function AdminClientesPage() {
  ...
  const accesos: AccesoCliente[] = ((accesosRes?.data ?? []) as any[]).map((r) => {
    ...
    return { id: r.id, nombre: r.nombre ?? '', email: r.email ?? '', passwordInicial: r.password_inicial ?? null, creadoEl: r.created_at ?? null, marcaNombre: m?.nombre ?? '—', marcaSlug: m?.slug ?? '', marcaEmoji: m?.emoji_marca ?? null }
  })
